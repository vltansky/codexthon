...
import { accessTokenFromHash } from "../src/access-session";
...
export function App() {
  ...
  useEffect(() => {
    ...
    if (!accessToken && !localPreview) void resolveUser();
  }, [accessToken, localPreview]);

  useEffect(() => {
    // Personal links navigate by fragment, so same-tab link changes do not reload the app.
    const controller = new AbortController();
    window.addEventListener("hashchange", () => setAccessToken(localPreview ? null : consumeAccessToken()), { signal: controller.signal });
